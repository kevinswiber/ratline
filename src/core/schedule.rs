...
pub struct TickSchedule {
    interval: Duration,
    /// The earliest instant the next child may spawn; `None` means now.
    due: Option<Instant>,
    in_flight: bool,
...
impl TickSchedule {
    ...
    pub fn new(interval: Duration) -> TickSchedule {
        TickSchedule {
            interval,
            due: None,
            in_flight: false,
...
mod tests {
    ...
    fn a_fresh_schedule_spawns_at_once() {
        let mut s = TickSchedule::new(IVL);
